    ...
    >
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-[88px] flex items-center justify-between p-3 pl-[1.4rem] z-[2]">
          <div
            className={`hamburger-menu ${isHamburgerOpen ? 'open' : ''} group h-full flex flex-col items-center justify-center cursor-pointer gap-[5px] order-2 md:order-none relative -translate-y-0.5`}
            onClick={toggleMenu}
            role="button"
          ...
          <Link
            to={ctaHref}
            className="card-nav-cta-button hidden md:inline-flex border-0 rounded-full px-5 items-center h-full font-medium cursor-pointer transition-all duration-300 relative -translate-y-0.5"
          >
            {ctaLabel}
          </Link>
        </div>
